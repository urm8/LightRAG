...
from lightrag.api.routers.log_routes import create_logs_routes
...
def create_app(args):
    ...
    if os.getenv("LIGHTRAG_MCP_ENABLED", "true").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    def create_role_llm_func(role: str, override_meta: dict | None = None):
        ...
        try:
            ...
            if role_binding == "anthropic":
                from lightrag.llm.anthropic import anthropic_complete_if_cache

                async def role_anthropic_complete(
                    prompt,
                    system_prompt=None,
                    history_messages=None,
                    **kwargs,
                ...
                return role_anthropic_complete
    ...
    app.include_router(create_logs_routes(api_key))
